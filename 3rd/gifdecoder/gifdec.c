...
gd_open_gif(const char *fname)
{
    ...
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
    gif->frame = calloc(4, (uint32_t)width * height);
    if (!gif->frame) {
        free(gif);
        goto fail;
    }
    gif->canvas = &gif->frame[(uint32_t)width * height];
    if (gif->bgindex)
        memset(gif->frame, gif->bgindex, (uint32_t)gif->width * gif->height);
    bgcolor = &gif->palette->colors[(uint32_t)gif->bgindex*3];
    if (bgcolor[0] || bgcolor[1] || bgcolor [2])
    {
        uint32_t total = (uint32_t)gif->width * gif->height;
        for (i = 0; i < total; i++)
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    frm_off = 0;
    ret = 0;
    frm_size = (uint32_t)gif->fw*gif->fh;
    while (frm_off < frm_size) {
        ...
        for (i = 0; i < str_len; i++) {
            ...
            gif->frame[(gif->fy + (uint32_t)y) * (uint32_t)gif->width + gif->fx + x] = entry.suffix;
...
read_image(gd_GIF *gif)
{
    uint8_t fisrz;
    int interlace;
    gd_Palette *old_palette = gif->palette;
    ...
    if (fisrz & 0x80) {
        ...
        gif->palette = &gif->gct;
    // palette changed, redraw background
    if (gif->palette != old_palette)
        dispose(gif);
    /* Image Data. */
...
render_frame_rect(gd_GIF *gif, uint8_t *buffer)
{
    int i, j, k;
    uint8_t index, *color;
    i = (uint32_t)gif->fy * gif->width + gif->fx;
    for (j = 0; j < gif->fh; j++) {
        for (k = 0; k < gif->fw; k++) {
            index = gif->frame[(gif->fy + j) * (uint32_t)gif->width + gif->fx + k];
            color = &gif->palette->colors[(uint32_t)index*3];
            if (!gif->gce.transparency || index != gif->bgindex)
...
{
    memcpy(buffer, gif->canvas, (uint32_t)gif->width * gif->height * 3);
...
{
    return gif->gce.transparency &&
         !memcmp(&gif->palette->colors[(uint32_t)gif->bgindex*3], color, 3);
}
